...
function OtherFunctions:GetPartsWeldedWith(basePart)
	ModuleComponents:params({ "Instance" }, { basePart }, 2)
...
function OtherFunctions:GetFullObjectMass(rootPart)
	ModuleComponents:params({ "Instance" }, { rootPart }, 2)
...
function OtherFunctions:VectorClamp(v, minVal, maxVal)
	ModuleComponents:params({ "Vector3", "number", "number" }, { v, minVal, maxVal }, 2)
	return Vector3.new(
...
function OtherFunctions:RandomVector(minVal, maxVal, seed)
	ModuleComponents:params({ "number", "number", "skip" }, { minVal, maxVal, seed }, 2)
	if seed ~= nil then
		ModuleComponents:assertf(typeof(seed) == "number", "seed must be number when provided")
	end
...
function OtherFunctions:ColorClamp(c, minVal, maxVal)
	ModuleComponents:params({ "Color3", "number", "number" }, { c, minVal, maxVal }, 2)
	return Color3.new(
